...
import Binarized.model as model
...
def train(epoch):
    ...
    # Update lr
    if epoch == 4 or epoch == 8 or epoch == 12 or epoch == 16 or epoch == 20 or epoch == 30 or epoch == 40 or epoch == 50:
        update_lr(optimizer)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/Binarized.ckpt')
        best_acc = acc
