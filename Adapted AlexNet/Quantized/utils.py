'''
Reference:https://github.com/eladhoffer/quantized.pytorch/blob/master/models/modules/quantize.py
'''
import torch
...
class UniformQuantize(InplaceFunction):
    ...
    @classmethod
    def forward(cls, ctx, input, num_bits, min_value=None, max_value=None,
...
class UniformQuantizeGrad(InplaceFunction):
    ...
    @classmethod
    def forward(cls, ctx, input, num_bits, min_value=None, max_value=None, stochastic=True, inplace=False):
        ctx.inplace = inplace
        ctx.num_bits = num_bits
        ctx.min_value = min_value
        ctx.max_value = max_value
...
def quantize(x, num_bits, min_value=None, max_value=None, num_chunks=None, stochastic=False, inplace=False):
    return UniformQuantize().apply(x, num_bits, min_value, max_value, num_chunks, stochastic, inplace)

def quantize_grad(x, num_bits, min_value=None, max_value=None, stochastic=True, inplace=False):
    return UniformQuantizeGrad().apply(x, num_bits, min_value, max_value, stochastic, inplace)

class QuantMeasure(nn.Module):
    """docstring for QuantMeasure."""

    def __init__(self, num_bits, momentum=0.1):
